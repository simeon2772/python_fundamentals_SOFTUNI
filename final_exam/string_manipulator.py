...
while True:
    ...
    if current_command == "End":
        ...
        translated_string = given_string.replace(char, replacement)
        ...
        if given_string.startswith(second_substring):
            print('True')
        else:
            print('False')
    elif current_command == 'Lowercase':
        lower_case_string = given_string.lower()
        print(lower_case_string)
    elif current_command == 'FindIndex':
        second_char = command[1]
        last_seen_index_char = given_string.rindex(second_char)
    elif current_command == 'Remove':
        start_index = command[1]
        count = command[2]
        new_string = given_string[int(start_index):int(count)]
        print(new_string)
